...
void playRandomTrack() {
  ...
  const byte nextTrack[]  = {0xAA, 0x06, 0x00, 0xB0}; // Correct next track command
  const byte vol20[]      = {0xAA, 0x13, 0x01, 0x14, 0xD2};

  sendCommand(randomMode, sizeof(randomMode));
  sendCommand(nextTrack, sizeof(nextTrack));
  sendCommand(vol20, sizeof(vol20));

  if (mp3Debug == 1) Serial.println("Random track started.");
}
