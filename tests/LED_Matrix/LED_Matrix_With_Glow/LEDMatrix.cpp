...
static void renderFireColumns() {
  ...
  // 3) Bidirectional diffusion (partial averaging)
  ...
  // 4) Update glowTrack (low-pass of heat) — “tube-warm” feel
  for (uint8_t col = 0; col < 32; ++col) {
    glowTrack[col] = lerp8by8(glowTrack[col], fireHeat[col], GLOW_TRACK_WEIGHT);
  }

  // 5) Flicker jitter — update less often for calmer highlights
  flickerTick++;
  bool updateJitterNow = (flickerTick % FIRE_FLICKER_PERIOD) == 0;
  if (updateJitterNow) {
    for (uint8_t col = 0; col < 32; ++col) {
