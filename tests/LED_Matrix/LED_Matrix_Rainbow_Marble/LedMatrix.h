...
namespace LedMatrix {
  ...
  // Folder 3: Christmas calm alternate pulse
  static const uint8_t  XMAS_MIN_BRIGHT         = 64;   // dim level
  static const uint8_t  XMAS_MAX_BRIGHT         = 255;  // bright level
  static const uint8_t  XMAS_PULSE_BPM          = 24;   // slow pulse (beats per minute)

  // Folder 4: Spooky (favor green) with slow breathing pulse
  static const uint8_t  SPOOKY_PULSE_BPM        = 10;   // very slow breathing
  static const uint16_t SPOOKY_TIME_NOISE_SPEED = 2;    // gentle evolution
  static const uint16_t SPOOKY_COLUMN_NOISE_SCALE = 3500; // smooth columns
