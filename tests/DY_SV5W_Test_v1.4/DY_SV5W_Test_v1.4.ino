...
void playRandomTrack() {
  const byte randomMode[] = {0xAA, 0x18, 0x01, 0x05, 0xC8}; // Enable random mode
  const byte vol20[]      = {0xAA, 0x13, 0x01, 0x14, 0xD2}; // Set volume to 20
  const byte playCmd[]    = {0xAA, 0x02, 0x00, 0xAC};       // Play command

  sendCommand(randomMode, sizeof(randomMode));
  sendCommand(vol20, sizeof(vol20));
  sendCommand(playCmd, sizeof(playCmd)); // Start playback

  if (mp3Debug == 1) Serial.println("Random mode enabled, volume set, and playback started.");
}
