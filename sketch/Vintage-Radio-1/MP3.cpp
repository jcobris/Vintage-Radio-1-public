...
#define MP3_RX 12   // DY-SV5W TX -> Arduino RX pin
#define MP3_TX 11   // DY-SV5W RX -> Arduino TX pin
...
// ------------ NEW: ASCII "AA 02 00 AC" -> HEX bytes ------------

// Convert a line like "AA 02 00 AC" into bytes and send to mp3Serial
void sendAsciiHexLineToMp3(const String &line) {
  String s = line;
  s.trim();
  ...
  while (idx < len) {
    ...
    // Convert ASCII hex (e.g. "AA", "2", "0F") to byte
    char buf[5];  // enough for "FF" + null
    token.toCharArray(buf, sizeof(buf));
    long value = strtol(buf, nullptr, 16);   // base 16
    byte b = (byte)value;
